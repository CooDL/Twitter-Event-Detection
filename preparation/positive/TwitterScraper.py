...
class TwitterSearch(metaclass=ABCMeta):
    ...
    def __init__(self, rate_delay, error_delay=5, lang='en'):
        """
    ...
    def parse_tweets(items_html):
        ...
        for li in soup.find_all("li", class_='js-stream-item'):
            ...
            if text_p is not None:# and (text_p['lang'] == 'en'):#
                tweet['text'] = text_p.get_text()
                tweet['lang'] = text_p['lang']
                print(tweet['text'])
                print(tweet['lang'])
    ...
    def construct_url(query, max_position=None):
        ...
        #log.info('URLSITE: %s'%parse.urlunparse(url_tupple))
...
class TwitterSearchImpl(TwitterSearch):
    ...
    def save_tweets(self, tweets):
        ...
        for tweet in tweets:
            ...
            if tweet['created_at'] is not None:# and (tweet['text'] is not None):
                t = datetime.datetime.fromtimestamp((tweet['created_at']/1000))
                fmt = "%Y-%m-%d %H:%M:%S"
                #log.info("%i [%s] - %s" % (self.counter, t.strftime(fmt), tweet['text'].replace('\n', ' ').encode('utf-8')))
                print(tweet['text'].replace('\n', ' '))#('['+t.strftime(fmt)+']||||||'+tweet['tweet_id']+'||||||'+tweet['text'].replace('\n', ' ').encode('utf-8'))
...
if __name__ == '__main__':
    ...
    search_query = "duhk encryption keys"
    ...
    select_tweets_until = datetime.datetime.strptime("2017-12-10", '%Y-%m-%d')
    threads = 10
